...
# Put it into action
def main():
    print('These are good movies: ')
    for movie in allgood(movies):
        print movie
    print

    print "The average rating of all listed movies is: "
    print round(avgScore(movies),2)
    print

    go = 'y'
    while go == 'y':
        cat = raw_input("Which category would you like details for? ").title()
        print ("The average rating of %s movies is: " %cat + str(round(avgScore(inCat(cat)),2)))
        print("The %s movies are: " %cat)
        for movie in inCat(cat):
            print movie['name']
        print
        go = raw_input("See another category? Type 'y' for yes. ")
